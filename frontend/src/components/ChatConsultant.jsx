...
const ChatConsultant = () => {
  ...
  const matchProperties = () => {
    const [budgetInput, cityInput, typeInput, lifestyleInput] = answers;

    const matched = mockProperties.filter((property) => {
      ...
      const budgetMatch = (() => {
        ...
        if (budgetInput.includes('-')) {
          const [min, max] = budgetInput.split('-').map(Number);
          return price >= min && price <= max;
        } else {
          const value = Number(budgetInput);
          return price >= value - 50000 && price <= value + 50000;
        }
      })();

      const cityMatch = cityInput ? location.includes(cityInput.toLowerCase()) : true;
      const typeMatch = typeInput ? summary.includes(typeInput.toLowerCase()) : true;
      const lifestyleMatch = lifestyleInput ? summary.includes(lifestyleInput.toLowerCase()) : true;

      return budgetMatch && cityMatch && typeMatch && lifestyleMatch;
    });
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center px-4 py-16 bg-cover bg-center font-['SF Pro Display']">
      {!showSuggested || Array.isArray(showSuggested) ? (
          ...
          >
            <div className="flex-1 overflow-y-auto space-y-4 pr-1 scrollbar-hide">
              {messages.map((msg, index) => (
                <div
                  key={index}
                  className={`max-w-[80%] px-4 py-3 rounded-lg text-sm md:text-base whitespace-pre-wrap flex items-start gap-2 ${
                    msg.role === 'ai'
                      ? 'bg-brand-seafoam text-black self-start shadow-lg'
